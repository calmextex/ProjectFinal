...
const char *WINDOWTITLE = "OpenGL / GLUT Sample -- Abraham Zamora";
...
struct planet Planets[] = {
	{ "Sun",     "sun.bmp",      15.0f,  sun,   pow(sun, 3./2.),  25.0f, 0, 0},
	{ "Mercury", "mercury.bmp",  0.38f,  mercury,   pow(mercury, 3./2.),  58.6f, 0, 0},
	{ "Venus",   "venus.bmp",    0.95f,  venus,  pow(venus, 3./2.), 243.0f, 0, 0},
	{ "Earth",   "earth.bmp",    1.0f,  earth,  pow(earth, 3./2.),   1.0f, 0, 0},
	{ "Mars",    "mars.bmp",     0.53f,  mars,  pow(mars, 3./2.),   1.03f, 0, 0},
	{ "Jupiter", "jupiter.bmp", 11.21f, jupiter,pow(jupiter, 3./2.),   0.41f, 0, 0},
	{ "Saturn",  "saturn.bmp",   9.45f, saturn, pow(saturn, 3./2.),  0.45f, 0, 0},
	{ "Uranus",  "uranus.bmp",   4.01f, uranus, pow(uranus, 3./2.),  0.72f, 0, 0},
...
const int MS_PER_CYCLE = 1;		// 10000 milliseconds = 10 seconds
...
const float PLANET_SIZE_SCALE = 1.f;     // Scale for planet sizes
const float ORBITAL_RADIUS_SCALE = 0.5f; // Scale for orbital radius
const float ORBITAL_PERIOD_SCALE = 365.0f / 365.0f; // Scale for orbital period (1 day as 1 year)
const float ROTATION_PERIOD_SCALE = 1000.0f / 24.0f; // Scale for rotation period (1 hour as 1 day)
...
Display( )
{
	...
	if( NowProjection == ORTHO )
		glOrtho( -255.f, 255.f,     -255.f, 255.f,     0.1f, 1000.f );
	else
	...
	for (int i = 0; i < NUM_PLANETS; i++)
	{
		glPushMatrix();
			...
			glPushMatrix();
				...
				glScalef(Planets[i].scale * PLANET_SIZE_SCALE, Planets[i].scale * PLANET_SIZE_SCALE, Planets[i].scale * PLANET_SIZE_SCALE);
				glBindTexture(GL_TEXTURE_2D, Planets[i].textureObject);
...
InitGraphics( )
{
	...
	// set textures and load textures for each planet
	glEnable(GL_TEXTURE_2D);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
...
InitLists( )
{
	...
	// initialize for each planet and sun
	for (int i = 0; i < NUM_PLANETS; i++) {
	...
	for (int i = 0; i < NUM_PLANETS; i++) {
		if (i != 0)
		{
			Planets[i].orbitDisplay = glGenLists(1);
			glNewList(Planets[i].orbitDisplay, GL_COMPILE);

			glColor3f(1.f, 1.f, 1.f);
			glBegin(GL_LINE_LOOP);
			for (int j = 0; j < numSegments; j++) {
				float angle = 2.f * M_PI * j / numSegments;
				float x = Planets[i].orbitalRadius * ORBITAL_RADIUS_SCALE * cos(angle);
				float z = Planets[i].orbitalRadius * ORBITAL_RADIUS_SCALE * sin(angle);
				glVertex3f(x, 0.f, z);
			}
			glEnd();
			glEndList();
